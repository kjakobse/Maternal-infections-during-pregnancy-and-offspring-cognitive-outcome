...
BuildSplineCIPlotCb <- function(
  ...
  if (!is_logical(return)) {
    stop("return must be TRUE/FALSE.")
  }
  ...
  if (cat_grp) {
    exp_dk <- exp_dk +
      ggplot2::scale_color_manual(
        values = c("#882255", desat("#882255", 0.5))
      )
  }
  exp_dk <- exp_dk +
  ...
  if (!is.null(plot_data_mat$ns_data)) {
    exp_mat <- exp_mat +
      ggplot2::geom_line(
        ...
        color = desat("#5ea5e8", 0.5),
        linewidth = 0.85
      ) +
      ggplot2::geom_ribbon(
        ggplot2::aes(x = day, ymin = conf.low, ymax = conf.high),
        data = plot_data_mat$ns_data,
        fill = desat("#5ea5e8", 0.5),
        alpha = 0.2
      )
  }
  if(!is.null(plot_data_mat$cat_data)) {
  ...
  if (cat_grp) {
    exp_mat <- exp_mat +
      ggplot2::scale_color_manual(
        values = c("#3aa5e8", desat("#5ea5e8", 0.5))
      )
  }
  ...
  if (!is.null(plot_data_bpp$ns_data)) {
    exp_bpp <- exp_bpp +
      ggplot2::geom_line(
        ...
        color = desat("#117733", 0.5),
        linewidth = 0.85
      ) +
      ggplot2::geom_ribbon(
        ggplot2::aes(x = day, ymin = conf.low, ymax = conf.high),
        data = plot_data_bpp$ns_data,
        fill = desat("#117733", 0.5),
        alpha = 0.2
      )
  }
  if(!is.null(plot_data_bpp$cat_data)) {
  ...
  if (cat_grp) {
    exp_bpp <- exp_bpp +
      ggplot2::scale_color_manual(
        values = c("#115533", desat("#117733", 0.5))
      )
  }
